#!/usr/bin/env python3
"""save_as: marcar el proyecto como sucio y reintentar el guardado con nombre.

FL ignora 'Save as' si no hay cambios sin guardar, asi que primero se toca
algo (tempo) para poner changed=1.
"""
import ctypes
import json
import os
...
def main():
    ...
    # 1. ensuciar el proyecto
    print("  >>>(setTempo 131) para marcar el proyecto como sucio")
    call("transport.setTempo", {"bpm": 131})
    time.sleep(0.4)
    m = call("project.metadata")
    print("  changed = %s" % (m["result"].get("changed") if m and m.get("ok") else "?"))
    ...
    if not dlg:
        print("  no abrio. Probando FPT_Save (Ctrl+S), que tambien abre el dialogo")
        call("meta.exec", {"code": "transport.globalTransport(midi.FPT_Save, 1)"})
        time.sleep(1.8)
        dlg = next((h for h, c, t in windows_of(pid) if h not in base), None)
    if not dlg:
        print("  SIGUE sin dialogo")
        return 1
    ...
    for i, k in enumerate(ks):
        ...
        if tras and cls_of(k) == "TQuickEdit":
            objetivo = k
            print("  campo nombre: TQuickEdit idx %d" % i)
            break
    if objetivo is None:
        objetivo = next((k for k in ks if cls_of(k) == "TQuickEdit"), None)
    if objetivo is None:
        print("  sin campo editable")
    ...
    if not ex:
        print("  ficheros en la carpeta de proyectos:")
        for f in sorted(os.listdir(PROJ))[:12]:
            print("    " + f)
